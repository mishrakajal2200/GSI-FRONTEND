...
//   const { clearCart, setUser } = useCart(); // assuming you have a function to reset cart context
...
//       // removed savedItems line to avoid error
//     }
...
//   if (searchTerm.trim()) {
//     try {
//       const response = await fetch(`https://gsi-backend-production.up.railway.app/api/getproducts/search?query=${encodeURIComponent(searchTerm.trim())}`);
//       if (response.ok) {
//         const result = await response.json(); // contains { success, products }
//         if (result.success) {
//           console.log("Search results:", result.products);

//           // Navigate with query param and pass search results to /shop page
